...
function init(): void {
    ...
    calcButtons.addEventListener('click', function(event: Event) {
...
function buttonClick(value: string): void {
    ...
    if (isNaN(Number(value)) && value !== ',') {
        handleSymbol(value as SymbolType);
    } else {
        handleNumber(value);
    }
...
function handleSymbol(symbol: SymbolType): void {
    const state = calculator.getState();
...
function handleMath(symbol: Operator): void {
    const state = calculator.getState();
...
function flushOperation(floatBuffer: number): void {
    const state = calculator.getState();
